...
def extract_venue_from_gs_a(gs_a_text: str) -> str:
    ...
    # Format is usually: "Journal Name, Year" or "Journal Name, Year - Publisher"
    venue_part = parts[1]
    
    # Remove the year and everything after (year is typically 4 digits)
    # Also handle the case where there might be a publisher after another dash
    venue_cleaned = re.split(r',\s*\d{4}', venue_part)[0].strip()
    
    # Remove common prefixes/suffixes that aren't part of the venue name
    venue_cleaned = venue_cleaned.strip('…').strip()
    
    # If the venue is empty or looks like a URL, return empty
    if not venue_cleaned or venue_cleaned.startswith('http'):
        return NO_VENUE_FOUND_STR
